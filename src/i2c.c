...
static void I2c_setI2cBusPinsToI2cMode(int32_t busNum);
static void I2c_getP9DataAndClockPinsForI2cBusNum(int32_t busNum,
                                                  int32_t *pPinNumsOut);
static void I2c_getBusNameStringFromBusNum(char *pBusNameBuffer,
                                           size_t bufferSize, int32_t busNum);
static void I2c_setI2cBusPinToI2cMode(const char *p9DataPinStr);
static int I2c_setI2CDeviceToSlaveAddress(const char *pBusName,
                                          int32_t deviceAddress);
static void I2c_writeRegAddrToI2cBus(int32_t i2cFileDesc, uint8_t regAddr);
static void I2c_readBytesFromRegAddr(int32_t i2cFileDesc, uint8_t *pValueOutput,
                                     size_t sizeofValueOutput);

int32_t I2c_initI2cDevice(int32_t busNum, int32_t deviceAddress)
{
  I2c_setI2cBusPinsToI2cMode(busNum);
  ...
  I2c_getBusNameStringFromBusNum(busName, BUS_NAME_BUFFER_SIZE, busNum);

  int32_t i2cFileDesc = I2c_setI2CDeviceToSlaveAddress(busName, deviceAddress);
...
static void I2c_setI2cBusPinsToI2cMode(int32_t busNum)
{
  ...
  I2c_getP9DataAndClockPinsForI2cBusNum(busNum, dataAndClockPins);
...
static void I2c_getBusNameStringFromBusNum(char *pBusNameBufferOut,
                                           size_t bufferSize, int32_t busNum)
{
  snprintf(pBusNameBufferOut, bufferSize, I2CDRV_LINUX_BUS_FORMAT, busNum);
}

static int32_t I2c_setI2CDeviceToSlaveAddress(const char *pBusName,
                                              int32_t deviceAddress)
{
...
void I2c_writeI2cReg(int32_t i2cFileDesc, uint8_t regAddr, uint8_t value)
{
  ...
  size_t bytesWritten = write(i2cFileDesc, buff, 2);
                    ...
                    size_t numBytesToRead)
{
  ...
  I2c_writeRegAddrToI2cBus(i2cFileDesc, regAddr);

  // Read numBytesToRead bytes into pBufferOut
  I2c_readBytesFromRegAddr(i2cFileDesc, pBufferOut, numBytesToRead);
}

static void I2c_writeRegAddrToI2cBus(int32_t i2cFileDesc, uint8_t regAddr)
{
  size_t bytesWritten = write(i2cFileDesc, &regAddr, sizeof(regAddr));
...
static void I2c_readBytesFromRegAddr(int32_t i2cFileDesc, uint8_t *pValueOut,
                                     size_t sizeofValueOutput)
{
  size_t bytesRead = read(i2cFileDesc, pValueOut, sizeofValueOutput);
